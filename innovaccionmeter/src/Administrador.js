...
export default class Administrador extends Component {
    ...
    link_reporte_empresa = (obj) => {        
        if (obj.obj.total_contestado > 0) {
            return (
                <u>
                    <a className="text-dark" onClick={() => this.reporte_empresa(obj.obj.Tipo, obj.obj.IdEmpresa)} href={process.env.REACT_APP_LINK}>{obj.obj.titulo}</a>
                </u>
            )
        } else {            
            return (
                <u>
                    <a className="text-dark" onClick={() => window.ModalError("Reporte","No hay datos")} href={process.env.REACT_APP_LINK}>{obj.obj.titulo}</a>
                </u>
            )
        }
    }
                    ...
                    {datos.map((obj, index) => {
                        return (
                            <div className="row text-center p-4" key={index}>
                                ...
                                <div className="col-6" style={{ "border": "1px solid #c9c9c9" }}>
                                    <u>
                                        <a className="text-dark" href={process.env.REACT_APP_LINK} onClick={() => this.lista_empleados(obj.Tipo, this.state.empresa)}>Ver encuestas</a>
                                    </u>
                                </div>
                            </div>
                        )
